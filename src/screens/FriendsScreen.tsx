...
import { Search, UserPlus, Check, X, UserMinus, Users, Clock } from "lucide-react";
...
// request; accept/decline incoming requests; see your friends. Backed by the
// Supabase social RPCs (migration 018).
export function FriendsScreen({ back }: { back: () => void }) {
  ...
  const add = async (u: UserResult) => {
    setRel(m => ({ ...m, [u.id]: "pending_out" }));
            ...
            {results.map(u => {
              ...
              return (
                <div className="friend-row" key={u.id}>
                  ...
                  {r === "none" && <button className="secondary sm" onClick={() => add(u)}><UserPlus size={15} />Add</button>}
                </div>
              );
            })}
          </div>
            ...
            <section className="friend-block">
              ...
              {incoming.map(r => (
                <div className="friend-row" key={r.id}>
                  <Avatar name={r.name} image={r.avatar} />
                  <b>{r.name}</b>
            ...
            {friends.length === 0
              ...
              : friends.map(f => (
                <div className="friend-row" key={f.id}>
                  <Avatar name={f.name} image={f.avatar} />
                  <b>{f.name}</b>
